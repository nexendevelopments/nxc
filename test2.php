...
$output = $host->instances->execute("nexenCloud", "cat /proc/loadavg", $record = true, [], true);
$averages = trim($host->instances->logs->read("nexenCloud", $output["output"][0]));
$x = explode(" ", $averages);
$dateTime = (new \DateTimeImmutable())->format("Y-m-d H:i:s");

$host->instances->logs->remove("nexenCloud", $output["output"][0]);
$host->instances->logs->remove("nexenCloud", $output["output"][1]);
var_dump([
    "1 minute"=>$x[0],
    "5 minutes"=>$x[1],
    "15 minutes"=>$x[2]
...
// $instances = ["c0-node0", "c0-node1", "c1-node1", "nexenCloud", "music"];
//
//
//
...
// // $p->store(14, "test", "wow");
//
// // $x = $client->containers->create("test", ["source"=>"/var/www/NexenCloud/src/sensitiveData/backups/14/test/backup.2019-10-19T21:19:24+01:00.tar.gz"]);
//
//
//
