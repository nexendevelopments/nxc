...
git clone https://github.com/turtle0x1/NexenCloud.git || exit $?

mkdir -p /var/www/NexenCloud/src/sensitiveData/certs
mkdir -p /var/www/NexenCloud/src/sensitiveData/backups
chown -R www-data:www-data /var/www/NexenCloud/src/sensitiveData/
...
# is not compatible with and so this is required to ensure NEXENCloud installs
# proprerly
if [ "$os_name" == "Ubuntu" ]
then
